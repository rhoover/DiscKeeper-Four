...
const listcourses = {
    ...
    fetchedCourses.forEach((course, duplicates) => {
      ...
      coursesOutput += `
        <div class="list-courses-item" data-courseid="${course.courseID}">
          <p class="list-courses-item-name">${course.courseName}</p>
          ...
          <pclass="list-courses-item-scored">Completed Rounds Scored:  ${course.roundsScored}</p>
        </div>
      `;
    });
  ...
  modalChoices(targetDOM, coursesModal, clickedCourseID, fetchedCourses, fetchedRounds) {
    ...
    function clickedButtonListener(event) {
      ...
      switch (buttonChoice) {
        ...
        case 'no':
          // listcourses.deleteCourse(targetDOM, clickedCourseID, fetchedCourses);
          coursesModal.classList.toggle('list-courses-modal-open');
        break;
        default:
        break;
      };
  ...
  successModal(fetchedCourses, clickedCourseID) {
    ...
    let deletedModal = document.querySelector('.list-courses-modal-deleted');
    deletedModal.classList.toggle('list-courses-modal-deleted-open');
    ...
    document.querySelector('.list-courses-modal-deleted-name').innerHTML = `${clickedCourseName}`;
