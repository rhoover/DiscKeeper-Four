...
const listplayers = {
  ...
  buildListForDOM(fetchedData) {
    ...
    fetchedData.forEach(player => {
      ...
      ${(() => {
       if (player.primary) {
        return `
        <div class="list-players-item-primary" data-playerID="${player.playerID}" data-primary="${player.primary}">
          <p class="list-players-item-header">App Owner:</p>
          <p class="list-players-item-primary-name">${player.nameFirst} ${player.nameLast}</p>
        </div>`
       } else {
        return `
        <div class="list-players-item-normal" data-playerID="${player.playerID}" data-primary="${player.primary}">
  ...
  }, // end buildListFor DOM

  deletePlayer(fetchedData) {

    //declare all the things
    let playerList = document.querySelector('.list-players-items');
    let ident = "";
    let indexOfPlayer;
    ...
    function playerListListener(event) {
      ...
      target.classList.add('list-players-item-normal-remove');
